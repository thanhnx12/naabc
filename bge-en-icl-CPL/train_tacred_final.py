    ...
    def train_model(self, encoder_llm, training_data, seen_des, seen_relations, list_seen_des, is_memory=False):
        ...
        for i in range(epoch):
            ...
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                loss = self.llm_moment.contrastive_loss(hidden, labels , is_memory)
                ...
                # update moment
                if is_memory:
                    self.llm_moment.update(ind, hidden.detach().cpu().data, is_memory=True)
                else:
                    self.llm_moment.update(ind, hidden.detach().cpu().data, is_memory=False)
                # print
                if is_memory:
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            if step > 0:
                ...
                self.llm_moment.init_moment(encoder_llm, memory_data_initialize, is_memory=True) 
                self.train_model(encoder_llm, memory_data_initialize, seen_des, seen_relations, list_seen_des, is_memory=True)
            ...
            ac1,ac1_des, ac1_rrf = self.eval_encoder_proto(encoder_llm,seen_proto_llm,seen_relid,test_data_initialize_cur)
            ac2,ac2_des, ac2_rrf = self.eval_encoder_proto(encoder_llm,seen_proto_llm,seen_relid,test_data_initialize_seen)
