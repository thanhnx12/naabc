...
class Manager(object):
    ...
    def _edist_cosine_proto_des(self, x1, x2, seen_embedding = None):
        '''
        input: x1 (B, H), x2 (N, H) ; N is the number of relations
        return: (B, N)
        '''
        x3 = torch.tensor(seen_embedding)
    ...
    def train_model(self, encoder, training_data, is_memory=False, relationid2cluster=None , seen_relation_embeddings = None, relationid2cluster_centroids = None ):
        ...
        if is_memory:
            optimizer = optim.Adam(params=encoder.parameters(), lr=0.000001)
        else:
        ...
        for i in range(epoch):
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                # loss3 = triplet(hidden, centroids_current,  cluster_centroids)

                # loss4 = triplet(hidden, centroids_current, nearest_cluster_centroids)
    ...
    def eval_encoder_proto(self, encoder, seen_proto, seen_relid, test_data, seen_embedding = None):
        ...
        for batch_num, (instance, label, _) in enumerate(test_loader):
            ...
            # logits = -self._edist_cosine_proto_des(fea, seen_proto, seen_embedding)
    ...
    def find_best_n_clusters(self, embeddings, max_clusters=10):
        max_clusters = len(embeddings)//4 
        best_n_clusters = 2
        best_score = -1
        ...
        for n_clusters in range(2, max_clusters + 1):
            ...
            # Update the best score and the number of clusters
            if score > best_score:
                best_score = score
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            num_clusters = self.find_best_n_clusters(seen_relation_embeddings)
...
if __name__ == '__main__':
    ...
    if config.task_name == 'FewRel':
        ...
        config.relation_embedding = './data/CFRLFewRel/fewrel_embeddings_chuan.pkl'
        if config.num_k == 5:
        ...
        config.relation_embedding = './data/CFRLTacred/tacred_embeddings_only_label.pkl'
