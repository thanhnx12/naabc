...
from add_loss import InClusterLoss, ClusterLoss
...
class Manager(object):
    ...
    def train_model(self, encoder, training_data, is_memory=False, relationid2cluster=None):
        ...
        for i in range(epoch):
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                # print("Losses: ", loss.item(), loss1.item())
                loss = loss + loss1 + loss2
                print("Losses: ", loss.item(), loss1.item(), loss2.item())
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                optimizer.zero_grad()
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            clusters = self.get_cluster(seen_relation_embeddings, num_clusters) # array (N)
            ...
            # if step > 0:
            self.train_model(encoder, training_data_initialize, relationid2cluster=relationid2cluster)
            ...
            if step > 0:
                ...
                # if step > 0:
                self.train_model(encoder, memory_data_initialize, is_memory=True, relationid2cluster=relationid2cluster)
