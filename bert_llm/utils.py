...
class Moment:
    ...
    def init_moment(self, encoder, dataset, seen_des, id2rel, is_memory=False, is_llm = False):
        ...
        datalen = len(dataset)
        if not is_memory:
            self.features = torch.zeros(datalen, self.config.encoder_llm_output_size if is_llm else self.config.encoder_output_size)
            self.features_des = torch.zeros(datalen, self.config.encoder_llm_output_size if is_llm else self.config.encoder_output_size)
            ...
            for step, (instance, labels, ind) in enumerate(data_loader):
                ...
                fea = hidden.detach().cpu().data
                fea_des = hidden_des.detach().cpu().data
                self.update_des(ind, fea, fea_des, is_memory)
    ...
    def contrastive_loss(self, x, labels, is_memory=False, des= None, relation_2_cluster = None):
        ...
        if relation_2_cluster is not None:
            ...
            neg = relation_match * (1.0 + 0.1 * t2) + (1.0 - relation_match) * 1.0
