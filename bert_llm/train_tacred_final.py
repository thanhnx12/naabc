...
class Manager(object):
    ...
    def train_model(self, encoder, encoder_llm, training_data, seen_des, seen_relations, list_seen_des, is_memory=False, is_llm = False):
        ...
        if is_llm:
            for i in range(epoch):
                ...
                for batch_num, (instance, labels, ind) in enumerate(data_loader):
                    ...
                    rep_des = encoder_llm(batch_instance['input']) # b, dim

                    loss2 = self.llm_moment.mutual_information_loss(hidden, rep_des, labels)

                    # loss2 = self.llm_moment.contrastive_loss_des(rep_des, hidden, labels, is_memory)
                    ...
                    if i>=8:
                        ...
                        loss1 = self.llm_moment.contrastive_loss(hidden, labels, is_memory)
            ...
            encoder_llm.eval()
            encoder_llm.vector_linear.train()
            ...
            for i in range(epoch):
                ...
                #         # hidden_des = hidden_des.detach().cpu()
                ...
                for batch_num, (instance, labels, ind) in enumerate(data_loader):
                    ...
                    with torch.no_grad():
                        hidden_llm = encoder_llm(instance['input']) # b, dim
                    hidden_llm_bert_size = encoder_llm.compress_to_bert_size(hidden_llm)

                    # batch_instance_llm = {'input': []}
                    # batch_instance_llm['input'] = [seen_des[self.id2rel[label.item()]]['input'] for label in labels]
                    # rep_des_llm = encoder_llm(batch_instance_llm['input']) # b, dim


                    distil_loss = self.moment.mutual_information_loss(hidden, hidden_llm_bert_size, labels, temperature=1.0)
#                   
...
# #                         loss = loss1 + 2*loss2 + 1*loss3 + 1*loss4 + 2*loss_distillation + 2*loss_distillation_des
#                         loss2 = self.moment.contrastive_loss_des(rep_des, hidden, labels, is_memory)

#                         loss = loss_distillation + loss_distillation_des + loss2 

#                     else:
#                         loss1 = self.moment.contrastive_loss(hidden, labels, is_memory)
#                         loss2 = self.moment.contrastive_loss_des(rep_des, hidden, labels, is_memory)

# #                         loss = loss1 + 2*loss2 + 2*loss_distillation + 2*loss_distillation_des
#                         loss2 = self.moment.contrastive_loss_des(rep_des, hidden, labels, is_memory)

#                         loss = loss_distillation + loss_distillation_des + loss2

                    loss1 = self.moment.contrastive_loss(hidden, labels, is_memory)
                    loss = loss1 + distil_loss
    ...
    def eval_encoder_proto_des(self, encoder, seen_proto, seen_relid, test_data, rep_des):
        ...
        for batch_num, (instance, label, _) in enumerate(test_loader):
            ...
            logits_rrf = self._cosine_similarity(fea, seen_proto)  # (B, N)
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            self.train_model(encoder, encoder_llm, training_data_initialize, seen_des, seen_relations, list_seen_des, is_memory=False, is_llm = True)
            ...
            self.train_model(encoder, encoder_llm, training_data_initialize, seen_des, seen_relations, list_seen_des, is_memory=False, is_llm = False)
