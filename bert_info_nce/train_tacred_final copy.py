...
class Manager(object):
    ...
    def train_model(self, encoder, training_data, seen_des, seen_relations, list_seen_des, is_memory=False):
        ...
        optimizer = SAM(encoder.parameters(), optimizer, lr=0.1, momentum=0.9)
    ...
    def train_model_sam(self, step, encoder, training_data, seen_des, seen_relations, list_seen_des, is_memory=False):
        data_loader = get_data_loader_BERT(self.config, training_data, shuffle=True)

        optimizer = optim.Adam(params=encoder.parameters(), lr=self.config.lr)
        optimizer = SAM(encoder.parameters(), optimizer, lr=self.config.lr) if step > 0 else optimizer
        ...
        for i in range(epoch):
            ...
            for batch_num, (instance, labels, ind) in enumerate(data_loader):
                ...
                loss2 = self.moment.mutual_information_loss(hidden, rep_des, labels)  # Recompute loss2
    ...
    def train(self):
        ...
        for step, (training_data, valid_data, test_data, current_relations, \
            ...
            if step>0:
                self.train_model_sam(step, encoder, training_data_initialize, seen_des, seen_relations, list_seen_des, is_memory=False)
